...
var functions={
    ...
    forgotPassword:async(req,res) => {
        try{
            Admin.findOne({contactNo:req.body.contactNo}).then(admin =>{
    ...
    //     });
    // }
    getContractor:function(req,res){
  ...
  getConsumer:function(req,res){
    Consumer.find({}).exec(function(err,consumers){
        if(err) throw err;
        if(consumers){
            res.send({success:true,msg:"consumers found",consumers:consumers});
        }else{
            res.send({success:false,msg:"consumers not found"});
        }

    });
},
getHardware:function(req,res){
  Hardware.find({}).exec(function(err,hardwares){
      if(err) throw err;
      if(hardwares){
          res.send({success:true,msg:"hardware owners found",hardwares:hardwares});
      }else{
          res.send({success:false,msg:"hardware owners not found"});
      }

  });
},
getLabour:function(req,res){
...
getTransporter:function(req,res){
